...
{
  auto scrubbed = scrubLineForCriteria( input, isNumber );
  std::vector< int > toReturn;
  for ( auto str : scrubbed )
  {
    toReturn.push_back( std::stoi( str ) );
  }
  return toReturn;
}
...
// criteria. Adjacent charcaters that fit the criteria will be grouped together into strings, and all the strings will
// be returned in a vector.
